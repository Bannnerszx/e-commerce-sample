...
import { fetchShipRouteCache } from "../actions";
import { useMobile } from "@/app/components/use-mobile";

// Guards a lat/lng-bearing object so malformed/missing coordinates never reach Leaflet
// (which would throw and blank the map). Ships/points that fail this are skipped, and the
// rest of the map still renders as normal.
const hasCoords = (o) =>
  o != null && Number.isFinite(o.lat) && Number.isFinite(o.lng);

// ── Ship SVG icon factory ──────────────────────────────────────────────────
...
// ── FIX: Unroll longitudes to prevent Dateline "Bending" ───────────────────
...
// ── FIX: Next.js Hot-Reload Map Cleaner ────────────────────────────────────
function MapCleaner() {
  const map = useMap();
  useEffect(() => {
    return () => {
...
// ── Zoom controller — syncs external buttons with the leaflet map ──────────
...
// ── FIX: Robust Auto-Framer for Unrolled Routes ────────────────────────────
function RouteFitter({ selectedShip, customRoute, isMobile }) {
  ...
  useEffect(() => {
    ...
    // Build the framing path and always guarantee it has been unrolled. The origin→dest
    // fallback is only usable when both endpoints have valid coordinates.
    ...
    // Manually calculate min/max bounding box parameters to avoid Leaflet wrapping glitches
    ...
    // Smart World-Wrapping Camera Alignment
    const currentCenterLng = map.getCenter().lng;
    const boundsCenterLng = (minLng + maxLng) / 2;
    
    // Find the perfect 360-degree multiplier matching where the user is looking
    ...
    const timeout = setTimeout(() => {
      ...
      map.flyToBounds(shiftedBounds, {
        ...
        maxZoom: isMobile ? 3 : 5, // Maintains essential continent framing context
        duration: 1.2,
      });
    }, 400);
...
export function LeafletMap({
  ...
  useEffect(() => {
    ...
    // A simple flag to prevent setting state if the user clicks away before the fetch finishes
    let isMounted = true; 

    async function loadRouteFromFirestore() {
      try {
        const docId = (selectedShip.name || "").replace(/[\s\/]/g, '_');
        if (!docId) { setCustomRoute(null); return; } // ship without a name: nothing to look up

        // 1. Fetch the route that your server already saved to Firestore
        const cachedData = await fetchShipRouteCache(docId);

        if (!isMounted) return;

        // 2. If data exists, draw it immediately. No API calls!
        if (cachedData) {
          if (cachedData.forecastPath && cachedData.forecastPath.length > 0) {
            const formattedPath = cachedData.forecastPath.map(pt => [pt.lat, pt.lng]);
            setCustomRoute(fixDatelineCrossing(formattedPath));
          } else if (cachedData.path && cachedData.path.length > 0) {
            const formattedPath = cachedData.path.map(pt => [pt.lat, pt.lng]);
            setCustomRoute(fixDatelineCrossing(formattedPath));
          } else {
            // The document exists, but the server hasn't finished calculating the path yet.
            setCustomRoute(null); 
          }
        } else {
          // No data in Firestore for this ship yet.
          setCustomRoute(null);
        }
      } catch (err) {
        console.error("Failed to load route from Firestore:", err);
      }
    }
  ...
  // Continuous loop offsets covering 5 continuous worlds for ultra-wide panning
  ...
  return (
    <MapContainer
      ...
      worldCopyJump={false} // Disabled to prevent jittery camera snapping
      maxBounds={[
        [-85.0511, -180000],
        [85.0511, 180000],
      ]}
    ...
    >
      ...
      {/* Render route lines across all map copies */}
      {selectedShip && worldOffsets.map((offset) => {
        ...
        if (!basePath || basePath.length === 0) return null; // no drawable route: skip, keep the ship pin
      ...
      {/* Render destination markers across all map copies */}
      ...
      {/* Render custom route waypoints across all map copies */}
