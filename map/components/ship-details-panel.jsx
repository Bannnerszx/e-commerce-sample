...
function VesselTypeBadge({ type }) {
  // Updated for light mode: softer backgrounds, darker text, subtle borders
  const colorMap = {
...
function ShipDetailContent({ ship, onClose, isMobile }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Header - hide on mobile, as it is handled by the accordion trigger */}
      {!isMobile && (
...
export function ShipDetailsPanel({ ship, onClose }) {
  ...
  // Auto-collapse accordion when a new ship is selected
  useEffect(() => {
    setIsExpanded(false);
  }, [ship?.id]);
  ...
  // Calculate the bounds center to determine where the destination pin falls.
  // Only fold in coordinates that are actually present/finite so a ship missing
  // origin/destination data doesn't crash the panel — it just falls back to the
  // default side.
  ...
  // If destination is to the East (right side of screen), place panel on the Left.
  // NOTE: "left-4" would sit exactly on top of the KPI sidebar (which spans
  // left-4 → ~272px wide). Pushing to left-[300px] clears the sidebar so the
  // two panels never overlap.
  const isDestOnRight = Number.isFinite(ship.destination?.lng)
    ? ship.destination.lng > centerLng
    : false;
  const desktopPositionClass = isDestOnRight ? "left-[300px]" : "right-4";
    ...
    return (
      <div className="fixed top-4 left-4 right-4 z-40">
        <div
          ...
          style={{
            // Light mode mobile floating box
        ...
        >
          {/* Inner wrapper clips accordion content to rounded corners */}
  ...
  // Desktop: dynamic floating side card
  return (
    <div
      className={cn(
        "absolute top-20 z-30 w-80 rounded-xl overflow-hidden shadow-2xl transition-all duration-500 ease-&lsqb;cubic-bezier(0.32,0.72,0,1)&rsqb;",
        desktopPositionClass
      )}
      style={{
        // Light mode desktop panel
